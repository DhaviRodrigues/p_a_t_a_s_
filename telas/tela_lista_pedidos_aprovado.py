...
def transicao_para_menu_adm(window, canvas, usuario_logado):
    """Realiza a transição da lista de pedidos aprovados de volta para o menu de administrador."""
...
def criar_tela_lista_pedidos_aprovado(window, canvas, usuario_logado):
    """
    Cria a interface gráfica que exibe uma lista rolável de pedidos de adoção aprovados.
    Para cada pedido, um card exibe o nome do animal e as informações do usuário.
    ...
    if not pedidos_aprovados: # Se não houver pedidos aprovados...
