...
def transicao_para_menu_adm(window, canvas, usuario_logado):
    """Realiza a transição da lista de pedidos pendentes de volta para o menu de administrador."""
...
def criar_tela_lista_pedidos_pendente(window, canvas, usuario_logado):
    """
    Cria a interface gráfica que exibe uma lista rolável de pedidos de adoção pendentes.
    Para cada pedido, um card exibe o nome do animal e as informações do usuário solicitante.
    Esta tela é destinada a administradores para análise dos pedidos.
    """
    from .modulos import pedidos
    from telas import tela_info_pedido_pendente
    ...
    pedidos_pendentes = pedidos.carregar_dados("pedidos_pendente.json") # Carrega os dados dos pedidos pendentes.
    ...
    if not pedidos_pendentes: # Se não houver pedidos pendentes...
        label_vazio = Label( # Cria um label informativo.
            frame_cards,
            text="Não há nenhum pedido de adoção pendente no momento.",
            bg="#44312D",
            fg="#EED3B2",
            font=("Poppins", 24)
        )
    ...
    placeholder_path = tools.relative_to_assets("TelaLista", "image_2.png") # Define o caminho para a imagem placeholder.
    ...
    for pedido in pedidos_pendentes: # Itera sobre cada pedido aprovado para criar um card.
        for animal in todos_animais: # Verifica se o animal do pedido existe na lista de animais.
            if animal.get("id") == pedido.get("id_animal"):
                ...
                card_canvas.tag_bind(
                    ...
                    lambda e: tools.fade_out(window,canvas,lambda: tela_info_pedido_pendente.criar_tela_info_pedido_pendente(window, canvas, usuario_logado, pedido, nome_arquivo_foto)))
