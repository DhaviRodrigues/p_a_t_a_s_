...
def transicao_para_menu_principal(window, canvas, usuario_logado):
    """Realiza a transição da lista de tratamentos para o menu principal do usuário."""
...
def transicao_para_menu_adm(window, canvas, usuario_logado):
    """Realiza a transição da lista de tratamentos para o menu de administrador."""
...
def criar_tela_lista_tratamento(window, canvas, usuario_logado):
    ...
    button_2 = Button( # Botão para voltar ao menu anterior (admin ou principal).
    canvas,
    ...
    canvas_lista.pack(side="left", fill="both", expand=True) # Empacota o canvas à esquerda, preenchendo o espaço.
    ...
    animais_em_tratamento = animalcrud.carregar_dados("animais_tratamento.json") # Carrega os dados dos animais em tratamento.
    ...
    if not animais_em_tratamento: # Se não houver animais em tratamento...
    ...
    for todos_animais in animais_em_tratamento: # Itera sobre cada animal para criar um card.
        img_botao = PhotoImage( # Carrega a imagem de fundo do card (que é um botão).
        ...
        card_canvas.tag_bind(tag_card, # Associa um evento de clique a todos os elementos com a tag.
            ...
            lambda: tela_editar_animal.criar_tela_editar_animal(window, canvas, usuario_logado, animal, "animais_tratamento.json") # Se for admin, vai para a tela de edição.
            if usuario_logado.get("adm") == True
            else tela_info_pet_tratamento.criar_tela_info_pet_tratamento(window, canvas, usuario_logado, animal,))) # Se não, vai para a tela de informações.
